...
// set đã duyệt bản tin này
	...
	$paystate = array(
		'unpay' => '<font color="green">Chưa than toán</font>',
...
else if ( 'orderdetail2' == $action) {
	$order = Table::Fetch('buynow', $id);
	$user = Table::Fetch('user', $order['user_id']);
	$team_orders = DB::LimitQuery('team', array('condition'=>array('id'=>$order['team_id'])));
	...
	$html = render('manage_ajax_dialog_orderdetail2');
	json($html, 'dialog');
}
	...
	json('Business Delete failed', 'alert'); 
		...
		need_auth('team');
		Table::Delete('buynow', $id);
		Session::Set('notice', "Đơn hàng {$id} đã được xóa");
		json(null, 'refresh');
}
