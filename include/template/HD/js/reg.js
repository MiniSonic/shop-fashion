...
$(function () {
    ...
    SubEmail: function () {
		...
		FocusAndSelect($("#emailsub")), !1) : IsValidEmail($("#emailsub").val()) ? ($.ajax({
            ...
            beforeSend: function () {
                $(".login_loading1").css("display", "block")
            },
            success: function (n) {
				var i = n.split("*");
            ...
            complete: function () {$(".login_loading1").hide(); $("#emailsub").attr("value","--Email đăng ký khuyến mãi--")}
    ...
    Init: function () {
        _JSRegister.ClickDangKy(), _JSRegister.ProvinceChange(), _JSRegister.SubmitRegisterPopup(), _JSRegister.CloseRegisterBox(), _JSRegister.EscapeKeyPressed(".register-popup"), _JSRegister.NumberOnly("#tbxPhone1")
    },
        ...
        $("form#RegisterForm .regist_btn1 a").unbind("click").click(function () {
            ...
            $.ajax({
                ...
                success: function (n) {
                    var i = n,
                        r = $("#RegisterPoint").val();
                    i == "1" ? ($(".mem-register .register-message").html("<b style='color:red'>Đăng ký thành công, bạn có thể tiến hàng MUA HÀNG và trò chuyện với mọi người!</b>"), alert("Đăng ký thành công, bạn vui lòng kiểm tra mail trong INBOX và SPAM để kích hoạt tài khoản"), $(".mem-register .register-message").css("display", "block"), $(".mem-register .register-message").css("color", "#292929"), _JSRegister.CloseRegisterBox(), location.href = location.href.replace("#", "")) : i == "3" ? ($(".mem-register .register-message").html("Email đã được sử dụng, hãy nhập email khác hoặc dùng chức năng quên mật khẩu."), $(".mem-register .register-message").css("display", "block")) : ($(".mem-register .register-message").html("Đăng ký không thành công, vui lòng kiểm tra lại thông tin."), $(".mem-register .register-message").css("display", "block"))
                },
				complete: function () {
