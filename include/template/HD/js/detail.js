...
function JsDealDetail() {
    var o = function () {
        $("#btnGopYSanPham").click(function () {
            ...
            n.length > 10 ? $("#userNotLogDiv").css("display") != "none" ? _login.ShowFormlogin() : t() : alert("Vui lòng nhập nội dung góp ý nhiều hơn 10 ký tự")
        })
    }, t = function () {
        $("#ifpost").unbind("load").load(function () {
            var n = $("#ifpost").contents().find("body").html(),
                r = "<span>Cảm ơn bạn đã gửi góp ý :) Hệ thống sẽ kiểm tra nội dung trước khi hiển thị.</span>",
                i = "<span>Xin lỗi, Server đang bận nên bạn không thể gửi góp ý vào lúc này. Vui lòng gửi lại sau!</span>",
...
(function (n) {
    ...
    LogInNow: function (n) {
        return $("#txtEmailPopup").val() == "" ? (alert("Vui lòng nhập địa chỉ Email"), FocusAndSelect($("#txtEmailPopup")), !1) : IsValidEmail($("#txtEmailPopup").val()) ? $("#txtPasswordPopup").val() == "" ? (alert("Vui lòng nhập mật khẩu"), FocusAndSelect($("#txtPasswordPopup")), !1) : ($.ajax({
            ...
            success: function (t) {
                var r, u;
                t == "0" ? (alert("Tài khoản đăng nhập không khớp."), FocusAndSelect($("#txtEmailPopup"))) : (r = t.split("*"), $("#userNotLogDiv").css("display", "none"), $("#userLogDiv").css("display", "block"), $("#ctm_email").html(t), r[0] != null && $("#ctm_email").html(r[0]), r[1] != null && r[1] * 1 > 0 && ($(".mail-note").attr("title", "Bạn có " + r[1] + " tin nhắn mới"), $(".mail-note .number").html(r[1]), $(".mail-note").attr("style", "display:inline-block;")), $("#pop_fail .popfail-log").css("display", "none"), n != null ? n() : (u = new JsDealDetail, u.AddNewCommentPL()), $(".login_loading_popup").hide(), _login.ClearFormlogin(), _login.CloseFormlogin(), CloseMark())
            },
            error: function () {
    ...
    SubmitFormAddCart: function () {
        $(".control_button").click(function () {
            ...
            if ($(this).parent().parent().find(".option_list").find(".img_ls li").each(function () {
                ...
                $(this).parent().parent().find(".op_select_notice").html("<i style='color:#E6572B;font-size:11px;'>Vui lòng chọn một tùy chọn (click vào hình ảnh) để đặt mua</i>");
                return
            }
            return $.ajax({
    ...
    TextQuantityChange: function () {
        $(".select_option input:text").keyup(function () {
            $(this).parent().parent().parent().parent().find(".op_select_notice").html(""), parseInt($(this).val()) > parseInt($(this).parent().find("#maxValue").val()) ? ($(this).val($(this).parent().find("#maxValue").val()), $(this).parent().parent().parent().parent().find(".op_select_notice").html("<i style='color:#E6572B;font-size:11px;'>Bạn không thể mua quá: <b>" + $(this).parent().find("#maxValue").val() + "</b> sản phẩm với lựa chọn đó</i>")) : parseInt($(this).val()) > 0 || $(this).val("0")
        })
    },
