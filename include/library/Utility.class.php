...
class Utility 
{
    ...
    static public function Option($a=array(), $v=null, $all=null)
    {
        $option = null;
        if ( $all ){
            $selected = ($v) ? null : 'selected="selected"';
        ...
        foreach( $a AS $key=>$value )
        {
            ...
            $selected = in_array($key, $v) ? 'selected="selected"' : null;
    ...
    static public function HumanTime($time=null, $forceDate=false)
    {
        ...
        if ( $forceDate || $interval > 3600 ){ //30*86400
            return strftime("Vào lúc %H:%M, ngày %d-%m-%Y",$time);
        ...
        }else if ( 10 >= $interval){// < 5 second
            return "Vừa mới gửi";
        }else{ // < 1 min
            return "${interval} giây trước đó";
        }
    }
